  ...
  module PlanExecutor
    ...
    def execute(plan, router: TuSketchupAgent::Router, base_request_id: nil)
      ...
      normalized[:steps].each_with_index do |step, index|
        ...
        # Every plan step is pinned to the state observed immediately before it.
        # Caller-provided guards are intentionally overridden to prevent stale
        # continuation from silently operating on a newer model state.
        ...
        unless response[:ok] == true
          return {
            ...
            error: {
              ...
              message: "Plan dừng tại step #{index + 1}: #{step[:step_id]}"
            },
            plan_id: plan_id,
