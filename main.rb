...
module TuSketchupAgent
  ...
  # ==========================================
  # Dispatch API Handlers - Layers & Tags (v1.3)
  # ==========================================
  ...
  def create_layer(args)
    ...
    layer = nil
    created_new = false
    with_operation(model, "AI - Create Layer") do
  ...
  def set_entity_layer(args)
    ...
    layer = nil
    with_operation(model, "AI - Set Entity Layer") do
  ...
  def set_layer_visibility(args)
    ...
    updated = {}
    with_operation(model, "AI - Set Layer Visibility") do
  ...
  # ==========================================
  # Dispatch API Handlers - Scenes & Camera (v1.3)
  # ==========================================
  ...
  def create_scene(args)
    ...
    page = nil
    with_operation(model, "AI - Create Scene") do
      existing = model.pages[name]
      page = existing || model.pages.add(name)

      if args["preset"]
        setup_camera_preset(page.camera, args["preset"], args["perspective"] == true)
...

    {
      ...
      scene: {
        name: page.name,
        page_id: page.entityID,
        camera: camera_metadata(page.camera)
      },
      model_revision: model_revision,
      mcp_revision: model_revision,
      protocol_version: PROTOCOL_VERSION,
  ...
  def activate_scene(args)
...

    {
      ...
      page_id: page.entityID,
      camera: camera_metadata(model.active_view.camera),
      protocol_version: PROTOCOL_VERSION,
      min_compatible_protocol_version: MIN_COMPATIBLE_PROTOCOL_VERSION
    }
  end
  ...
  def set_camera_view(args)
    ...
    if args["preset"]
      setup_camera_preset(camera, args["preset"], args["perspective"] == true)
